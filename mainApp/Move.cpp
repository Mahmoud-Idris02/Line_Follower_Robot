...
void moveRight(unsigned int angle) {
  ...
  digitalWrite(backwardR, LOW);
  digitalWrite(backwardL, LOW);
  analogWrite(en1, leftSpeed + 60);
  analogWrite(en2, RightSpeed - 20);
}
...
void moveLeft(unsigned int angle) {
  ...
  analogWrite(en1, leftSpeed +50 );
  analogWrite(en2,  RightSpeed+50);
}
...
void move( unsigned int angle,  bool _90d , int state) {
  ...
  if (state == 0){
    ...
    if (_90d == true){spinRight(angle);}
    else if (_90d == false){moveRight(angle);}

  } else if (state == -1) {
    if (_90d == true){spinLeft(angle);}
    else if (_90d == false){moveLeft(angle);}
  }
}
